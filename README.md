...
- **指定時刻に自動実行**（3:00 / 9:00 / 15:00 / 21:00・日本時間）できる（PC の起動中のみ。Mac は launchd、Windows はタスクスケジューラで設定）。Mac の定期実行は **ターミナルを開かず**実行され、手動実行と同様に **ブラウザ（Chromium）が表示**されて取得します。
...
Mac が起動している間だけ、**日本時間 3:00 / 9:00 / 15:00 / 21:00** に自動実行する設定です。  
**指定時刻に Mac が起動・ログインしており、スリープでないこと**が必要です（スリープ中は実行されません）。
...
何かしら表示されれば登録されています。  
あとは指定時刻（3:00 / 9:00 / 15:00 / 21:00）に、Mac が起動していれば自動で実行されます。
...
**Mac:** 指定時刻のあとに `logs/output.log` を開き、実行時刻あたりに「取得日時: YYYY-MM-DD HH:MM」のような行が追記されているか確認する。`launchctl list com.cme.scraper` で登録されているか確認する。

**Windows:** タスクスケジューラで該当タスクが「有効」か確認する。指定時刻のあとに `logs\output.log` を開き、実行時刻あたりにログが追記されているか確認する。

### 指定時刻に実行されない（Mac）

- **スリープ中は動きません。** 指定時刻に Mac が起動・ログインしており、ディスプレイがオフでもスリープしていない状態である必要があります。
- **省電力モード** をオンにしていると、実行がスキップされることがあります。その時刻だけオフにするか、電源アダプタを繋いで試してください。
- タイムゾーンが「東京」になっているか、「システム設定」→「一般」→「日付と時刻」で確認してください。
- plist を変更したあとは、`launchctl unload` → `cp` → `launchctl load` で入れ直さないと反映されません。
...
- **Mac の時刻**  
  定期実行の時刻は **Mac のシステムのタイムゾーン** に従います。日本時間で動かす場合は、「システム設定」→「一般」→「日付と時刻」でタイムゾーンを **東京** にしておいてください。

- **スリープ・省電力**  
  指定時刻に Mac がスリープや省電力モードのときは、実行されないことがあります。確実に動かしたい時刻は、その前から電源を繋ぐか省電力モードをオフにしておくとよいです。
