...
#### 2-4. 「Probabilities」タブをクリック（140-165行目）
...
**なぜ複数のブラウザを試すのか：**
- ブラウザによって、サイトの表示が異なる場合がある
- Chromiumでエラーが出ても、Firefoxで成功することがある
- より高い成功率を確保するため

### 3. データ処理・書き込み（310-477行目）

**何をしているか：**
取得したデータを処理して、前回値と比較し、スプレッドシートに書き込みます。

#### 3-1. スプレッドシートを開く（311-314行目）

```python
spreadsheet = gc.open("CME定期調査")  # スプレッドシートを開く
sh = spreadsheet.sheet1  # 1つ目のシート（メインシート）を取得
now = datetime.now().strftime("%Y-%m-%d %H:%M")  # 現在時刻を取得
```

#### 3-2. 「前回値」シートの準備（316-336行目）
...
**`col_num_to_letter()` 関数（338-346行目）：**
